...
function Section({ letter, total, y, selectedId, onSelect, plotData, onHover, onHoverEnd }) {
  ...
  const rightEdge = LEFT_EDGE + cols * PLOT_W;
  const capApexX = rightEdge + CAP_W;
  const capMidY = y + SECTION_H / 2;
  const capId = `${letter}-${total}`;

  const capPoints = [
    `${rightEdge},${y}`,
    `${rightEdge},${y + SECTION_H}`,
    `${capApexX},${capMidY}`,
  ].join(' ');
  ...
  return (
    <g className={`section section-${letter}`}>
      <g data-testid="plot" onClick={e => onSelect(capId, e.clientX, e.clientY)} onMouseEnter={e => onHover(capId, e.clientX, e.clientY, capData.assignee)} onMouseLeave={onHoverEnd} style={{ cursor: 'pointer' }}>
        ...
        />
        <text x={rightEdge + CAP_W * 0.28} y={capMidY} textAnchor="middle" dominantBaseline="middle" className="cap-letter">
          {letter}
        </text>
        <text x={rightEdge + CAP_W * 0.28} y={capMidY + 13} textAnchor="middle" dominantBaseline="middle" className="cap-num">
          {capId}
        </text>
        {capData.assignee && (
          <text x={rightEdge + CAP_W * 0.28} y={capMidY + 25} textAnchor="middle" dominantBaseline="middle" className="plot-assignee">
            {capData.assignee}
          </text>
        )}
        {capData.flagged && (
          <g data-testid="flagged-icon" transform={`translate(${rightEdge + 3}, ${y + 3})`} style={{ pointerEvents: 'none' }}>
            <polygon points="5,0 10,9 0,9" fill="#e07b2a" />
            <text x="5" y="8" textAnchor="middle" fontSize="6" fontWeight="bold" fill="white">!</text>
          </g>
        )}
...
function SidePanel({ plotId, saved, onSave, onCancel, onClear, pos }) {
  ...
  return (
    <aside data-testid="side-panel" className="nwb-side-panel" style={{ left, top }}>
      <h2 className="nwb-panel-title">{plotId}</h2>
      <form onSubmit={e => { e.preventDefault(); onSave(draft); }}>
        <div className="nwb-field-row">
          <label htmlFor="nwb-occupancy">Occupancy</label>
          <select id="nwb-occupancy" value={draft.occupancy}
        ...
        <div className="nwb-field-row">
          <label htmlFor="nwb-flagged">Flagged</label>
          <input id="nwb-flagged" type="checkbox" checked={draft.flagged || false}
            onChange={e => set('flagged', e.target.checked)} />
        </div>
        <div className="nwb-field-row">
          <label htmlFor="nwb-assignee">Assignee</label>
          <input id="nwb-assignee" type="text" value={draft.assignee}
            onChange={e => set('assignee', e.target.value)} />
        </div>
        <div className="nwb-field-row">
          <label htmlFor="nwb-payment">Payment</label>
          <select id="nwb-payment" value={draft.payment}
        ...
        <div className="nwb-field-row">
          <label htmlFor="nwb-notes">Notes</label>
          <input id="nwb-notes" type="text" value={draft.notes}
            onChange={e => set('notes', e.target.value)} />
        </div>
        <div className="nwb-panel-actions">
...
export default function NorthwestBlock({ plotData = {}, onUpdate = () => {}, onClear = () => {}, onHover = () => {}, onHoverEnd = () => {} }) {
  ...
  const svgWidth = LEFT_EDGE + 7 * PLOT_W + CAP_W + 20;
  ...
  return (
    <div className="nwb-root">
      <style>{`
        ...
        .nwb-wrap svg { display: block; }
        .nwb-wrap .section text {
          fill: #2d4a1a;
          font-size: 11px;
        ...
        .nwb-wrap .cap-num { font-size: 9px !important; }
        .nwb-side-panel {
          position: fixed;
          z-index: 10000;
          background: #f5f0e8;
        ...
        .nwb-field-row input, .nwb-field-row select {
          font-family: 'Courier New', monospace;
          font-size: 12px;
          color: #2d4a1a;
          background: #ede8dc;
        ...
        .nwb-panel-actions { display: flex; gap: 8px; margin-top: 14px; }
        .nwb-panel-actions button {
          font-family: 'Courier New', monospace;
          font-size: 11px;
          padding: 4px 12px;
          border: 1.5px solid #5a7a3a;
        ...
        .nwb-panel-actions button:hover { background: #8fbe5a; }
      `}</style>
      <div className="nwb-wrap">
        <svg
